...
// Flow-matching decode steps. The upstream demo ships 1 (fastest, but flat/robotic). More
// steps integrate the flow ODE more finely -> smoother, more natural prosody & timbre, at ~Nx
// the decode compute. Affordable now that the WASM runs multi-threaded (see vite.config's
// cross-origin isolation). Lower this back toward 1 if synthesis ever drops below real-time
// and the voice starts breaking again.
const LSD_STEPS = 4;
...
// Model weights are loaded from Hugging Face at runtime (int8 bundle, ~180MB, cached by the
// browser after first load) so the ~180MB of weights don't have to live in this repo. For a
// fully offline / production setup, download the `onnx/<language>/` bundle into
// frontend/public/pockettts/onnx/ and change this back to `./onnx/${language}`.
const MODEL_BASE_URL = "https://huggingface.co/spaces/KevinAHM/pocket-tts-web/resolve/main/onnx";
