...
# Cache for the dashboard stats. Two layers keep it cheap at any scale:
...
_stats_cache: dict | None = None  # {checked_at, fingerprint, value}
_stats_lock = threading.Lock()


def _stats_fingerprint(db) -> str:
    """A cheap signal that moves whenever the stats could change, without scanning:
    - MAX(candidates.updated_at): moves on every candidate insert/update.
    - COUNT of interview results that have a final decision: moves when post-interview
    ...
    decided = (
        ...
        .filter(InterviewResult.decision.isnot(None))
        .scalar()
        or 0
    )
...
def recruitment_stats(current_user: User = Depends(get_current_user)) -> RecruitmentStats:
    ...
    with _stats_lock:
        _stats_cache = {"checked_at": time.monotonic(), "fingerprint": fingerprint, "value": value}
    return value
...
def _compute_page_analytics(page: int, page_size: int) -> PageAnalytics:
    """Two grouped scans, scoped to just this page of recruitments, build the per-recruitment
    funnel bars and the mutually-exclusive pipeline breakdown for the pie."""
    with session_scope() as db:
        total = db.query(func.count(Recruitment.id)).scalar() or 0
        page_recruitments = (
